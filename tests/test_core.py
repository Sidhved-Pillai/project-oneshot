...
from src.access_control import FULL_DELETE_USERS, PRIVATE_RECORD_USERS, SELF_DELETE_USERS, can_delete_record, can_view_record, can_view_trip_leaderboard, scope_report_rows
...
def test_record_delete_permissions_are_owner_scoped_for_manish():
    ...
    assert FULL_DELETE_USERS == {"Sid", "Ajit"}
    assert can_delete_record("Ajit", manish_record)
    assert can_delete_record("Ajit", {"created_by": "Nikhat"})
...
def test_vijay_transporter_profiles_fill_exact_bank_details():
    assert list(VIJAY_TRANSPORTER_PROFILES) == ["Altaf Khan Transport", "Nisar Anwar Shaikh"]
    altaf = vijay_transporter_profile("Altaf Khan Transport")
    nisar = vijay_transporter_profile("Nisar Anwar Shaikh")
    assert altaf == {
...
def test_vijay_revenue_dropdown_maps_to_transporter_freight():
    assert VIJAY_FREIGHT_RATES == {
        3844: 3270, 4509: 3862, 5073: 4300, 6509: 5500,
        8748: 7440, 10160: 8640, 4327: 3680,
    }
    assert vijay_transporter_freight(3844) == 3270
...
def test_bisleri_invoice_validation_rejects_dates_and_other_document_ids():
    assert valid_bisleri_invoice_number("MUMCIN270054834") == "MUMCIN270054834"
    assert valid_bisleri_invoice_number(" mumcin270054834 ") == "MUMCIN270054834"
    assert valid_bisleri_invoice_number("27-04-2026") == ""
    assert valid_bisleri_invoice_number("MUMC002700") == ""
    assert verified_bisleri_invoice_numbers([
        "27-04-2026", "MUMCIN270054834", "ACK-122634164886506",
    ]) == ["MUMCIN270054834"]
