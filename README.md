...
The web app is protected by a special-member access code before any operational data is loaded. The current code identifies Sid and stores that identity plus special-member status in the authenticated session. To rotate Sid's code without changing source code, set `SPECIAL_ACCESS_CODE` in `.env` locally or in Streamlit Cloud's encrypted Secrets. Never commit an access code to Git.
